...
# GDK_BACKEND intentionally NOT set — let the system pick Wayland or X11.
# The upstream default (GDK_BACKEND=x11) breaks Wayland-only compositors like Hyprland.
...
# Remove GPU/display libraries that MUST come from the host system.
# Bundling these from the build machine (e.g. Debian) causes EGL_BAD_PARAMETER
# crashes on systems with different Mesa versions (e.g. Arch with Mesa 25+).
echo "Removing bundled EGL/Mesa/Wayland/DRM libraries (must use host system versions)"
find "$APPDIR" -type f \( \
    -name "libEGL*" -o \
    -name "libGL.so*" -o \
    -name "libGLX*" -o \
    ...
    -name "libgbm*" -o \
    -name "libdrm*" -o \
    -name "libwayland-client*" -o \
    -name "libwayland-egl*" -o \
    -name "libwayland-cursor*" -o \
    -name "libwayland-server*" -o \
    -name "libvulkan*" \
\) -print -delete || true
