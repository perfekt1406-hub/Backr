...
fn ssh_control_socket_path(host: &str, ssh_port: u16) -> Result<String, BackrError> {
    ...
    // Available chars for the host segment: keep total filename ≤ 80 chars to stay under
    // the 104-char Unix socket path limit (macOS) when combined with the directory prefix.
...
/// unavailable, the ControlMaster options are silently omitted — multiplexing is a performance
